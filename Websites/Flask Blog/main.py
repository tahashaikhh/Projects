...
def home():
    ...
    return render_template('homepage.html', params=params, posts=posts, prev=prev, next=next)

@app.route("/about")
def about():
    return render_template('aboutpage.html', params=params)
...
def contact():
    if(request.method=='POST'):
        ...
        try:
            ...
            pass
    return render_template('contactpage.html', params=params)
...
def edit(sno):
    if('user' in session and session['user'] == params['admin_user']):
        if(request.method=='POST'):
            ...
            if(sno==0):
                post = Posts(title = p_title, slug = post_slug, content = post_content, img_file = img_file)
                db.session.add(post)
                db.session.commit()
            else:
